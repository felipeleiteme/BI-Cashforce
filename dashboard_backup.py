...
if df.empty:
    st.error("⚠️ Nenhum dado disponível. Verifique a conexão com o Supabase.")
    st.stop()
...
with tab1:
    ...
    if 'parceiro' not in df_filtered.columns or df_filtered.empty:
        st.warning("⚠️ Nenhum dado disponível para os filtros selecionados. Tente ajustar o período ou os filtros.")
    else:
        ...
        if not parceiros_com_dados:
            st.warning("⚠️ Nenhum dado encontrado para os parceiros selecionados no período escolhido. Tente ajustar os filtros.")
        elif len(parceiros_com_dados) < len(selected_parceiros):
            parceiros_sem_dados = set(selected_parceiros) - set(parceiros_com_dados)
            st.info(f"ℹ️ Os seguintes parceiros não possuem dados no período selecionado: {', '.join(parceiros_sem_dados)}")
        ...
        # Tabela Comparativa
        st.markdown("#### 📋 Tabela Comparativa de Parceiros")
...
with tab2:
    ...
    st.markdown("#### 🗺️ Mapa de Grupos Econômicos (Treemap)")
    if 'grupo_economico' in df_filtered.columns and 'valor_bruto_duplicata' in df_filtered.columns:
...
<div style='text-align: center; color: #7f8c8d; padding: 1rem;'>
    <p><strong>BI Cashforce Dashboard</strong> | Desenvolvido com ❤️ usando Streamlit & Plotly</p>
    <p style='font-size: 0.9rem;'>Dados atualizados em tempo real via Supabase</p>
</div>
""", unsafe_allow_html=True)
